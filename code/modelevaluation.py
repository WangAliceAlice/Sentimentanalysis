...
if __name__ == '__main__':
	data=[]
	labels=[]
	with open ("LSTM_data/train_data.txt","r",encoding="gbk",errors="ignore")as file:
		for line in file:
			line=line[0:1]
			labels.append(line)
	with open("LSTM_data/train_data.txt","r",encoding="gbk",errors="ignore")as file:
	...
	x_train, x_test, y_train, y_test= train_test_split(x, movie_target, test_size = 0.2) #随机划分训练集和测试集
	x_train = count_vec.fit_transform(x_train)  #数据预处理和数据转换
